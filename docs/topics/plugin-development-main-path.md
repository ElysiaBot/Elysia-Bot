...
- 当前主路径按这个顺序走：
  ...
  3. `npm run plugin:manifest:write -- -plugin ./plugins/plugin-example`
  4. `npm run plugin:manifest:check -- -plugin ./plugins/plugin-example`
  5. `npm run plugin:package -- -plugin ./plugins/plugin-example`
  6. 跑 smoke。仓库当前聚焦 smoke 入口是 `npm run test:plugin-template:smoke`，新 scaffold 出来的 repo-local 插件也先跑 `go test ./plugins/plugin-example`
...
- repo-local `plugin:scaffold -> 改 Manifest()/逻辑 -> plugin:manifest:write -> plugin:manifest:check -> plugin:package -> smoke`，是当前默认插件开发主路径。
- `plugin-template-smoke`、`plugin-dev` 命令面、以及 `npm run test:plugin-template:smoke` 一起构成当前仓库对这条路径的默认支持。

### Smoke / non-default

- 模板 smoke 当前用进程内 `DirectPluginHost` 证明 runtime register / dispatch / reply 合约成立，但这不等于完整 subprocess host 生命周期已经成为插件开发默认入口。
...
- 本地开发时，最直接的命令链是：`plugin:manifest:write`、`plugin:manifest:check`、`plugin:package`、`go test ./plugins/<plugin-id>`。
- 模板与主路径回归时，优先跑 `npm run test:plugin-template:smoke`。按 `package.json` 当前定义，这条命令会先执行 `manifest check`，再跑模板 smoke 测试。
